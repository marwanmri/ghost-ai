...
Also ceratain shapes are not representative. Check `context/screenshots/image.png` for the current brocken state.

Read the current canvas component code in `components/editor` and `components/canvas`.

Canvas space issues:
The etire 'CanvasStatusOverlay' is redundant in `components/collaborative-canvas.tsx`
