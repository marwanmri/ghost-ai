...
Use Clerk's `dark` theme from `@clerk/ui/theme` as the base.
...
Add Clerk's built-in `UserBottun` to the editor navbar right section for profile settings and logout.

Keep Clerk's default user menu and lows intact. Do not rebuild or heavily customize Clerk internals.
...
- auth pages use CSS variable with no hardcoded colors
